...
uint16 varRegisterMeasurement = 0;
//Add ADC variables
...
void core0_task_10ms(void)
{
    Total_Pjt_U.pushSW = IfxPort_getPinState(SW2);
    Total_Pjt_U.lightSensor = readADCValue(CHN_4);
    Total_Pjt_U.rotRegister = readADCValue(CHN_5);

    Total_Pjt_step();

    setDutyCycle(Total_Pjt_Y.blueLed);
    if (Total_Pjt_Y.redLed == 1)
    {
        IfxPort_setPinHigh(RED_LED);
    }
    else
    {
